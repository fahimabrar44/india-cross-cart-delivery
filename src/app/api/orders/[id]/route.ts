...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const data = await Order.findById(id)
      .populate('customer', 'name phone email address district')
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const allowedFields = ['status', 'paymentStatus', 'courierName', 'trackingNumber', 'dispatchDate', 'deliveryDate', 'notes', 'discount', 'shipping', 'paidAmount']
